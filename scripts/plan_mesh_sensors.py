...
def main():
    ...
    cfg = SimulationConfig(
        **json.loads(args.table.with_suffix(".tbl.json").read_text())["config"]
    )
                ...
                rows.append(
                    {
                        ...
                        "dx": 2
                        / (
                            snapshot["parameters"]["base_n"]
                            * 2 ** top["candidate_level"]
                        ),
                    }
                )
    for t in args.times:
        for eta in (0.0, 0.5, 0.8256):
            for theta in (0.0, np.pi / 4):
                xyz, wave = phase_geometry(
                    t, np.array(0.62), np.array(eta), np.array(theta), cfg
                )
                level = containing_level(xyz, blocks)
                dx = 2 / (snapshot["parameters"]["base_n"] * 2**level)
                rows.append(
                    {
                        "label": f"phase-{t}-{eta}-{theta:.4f}",
                        "time": t,
                        "xyz": xyz.tolist(),
                        "dx": dx,
                        "candidate_level": level,
                        "sampled_doubled_phase_points": float(np.pi / (dx * wave)),
                        "selection": "X=0.62; analytic support sensor, not necessarily evolved maximum; center spacing only",
                    }
                )
